...
#include "dji_sdk_demo/flight_control_gps.h"
...
{
  ros::init(argc, argv, "flight_control_gps_node");
  ros::NodeHandle nh;
  ...
  if(takeoff_result)
  {
    ...
    square_mission.setTarget(0, 5, 3, 60);
  ...
  localOffsetFromGpsOffset(localOffset,home_gps_position,current_gps);
...
M100monitoredTakeoff()
{
  ...
  land_gps_position = current_gps;                    //log takeoff position
  if(!takeoff_land(dji_sdk::DroneTaskControl::Request::TASK_TAKEOFF))
  {
    return false;
  }
  ...
  if(flight_status != DJISDK::M100FlightStatus::M100_STATUS_IN_AIR ||
      current_gps.altitude - land_gps_position.altitude < 1.0)                  //if height less than 1m then take off false
  {
  ...
  {
    start_time = ros::Time::now();
    home_gps_position = current_gps;          //log home position
    ROS_INFO("Successful takeoff!");
    ros::spinOnce();
  }
...
M100monitoredLand()
{
  ...
  // Step 1: If M100 is not in the air after 10 seconds, fail.
  while ((ros::Time::now() - start_time < ros::Duration(10))&&(current_gps.altitude - land_gps_position.altitude > 0.1))
  {
    ros::Duration(0.01).sleep();
    ros::spinOnce();
  }
  ...
  if(current_gps.altitude - land_gps_position.altitude > 1.0)                         // if height > 1m then landing false
  {
    ROS_INFO("c:%f,l:%f,state:%d",current_gps.altitude,land_gps_position.altitude,flight_status);
...
M100monitoredGoHome()
{
  ...
  // Step 1: If M100 is not in the air after 10 seconds, fail.
  while ((ros::Time::now() - start_time < ros::Duration(10))&&square_mission.state!=0)
  {
    ros::Duration(0.01).sleep();
    ros::spinOnce();
  }
