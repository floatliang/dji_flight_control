...
#if SIMULATION	    
     APs.resize(8);
    APs[0] = Eigen::Vector3d(10, -10, 10);
    APs[1] = Eigen::Vector3d(-10, 10, 10);
    APs[2] = Eigen::Vector3d(-10, -10, -10);
    APs[3] = Eigen::Vector3d(10, 10, -10);
    
    APs[4] = Eigen::Vector3d(10, 10, 10);
    APs[5] = Eigen::Vector3d(-10, 10, -10);
    APs[6] = Eigen::Vector3d(-10, -10, 10);
    APs[7] = Eigen::Vector3d(10, -10, -10);
    p = Eigen::Vector3d(10 + 0.5, 10 + 0.5, 10 + 0.5);
#else 	
	APs.resize(3);
	APs[0] = Eigen::Vector3d(0, -2.2, 0);
    APs[1] = Eigen::Vector3d(0, 0, 0);
    APs[2] = Eigen::Vector3d(2.8, 1.0, 0);
...
//    APs[6] = Eigen::Vector3d(-10, -10, 10);
//    APs[7] = Eigen::Vector3d(10, -10, -10);
    p = Eigen::Vector3d(0, -1.0, 0.0);
#endif 
    
    //P.block<3, 3>(9, 9) = 0.001 * Eigen::Matrix3d::Identity();
    P.block<3, 3>(9, 9) = 0.00001 * Eigen::Matrix3d::Identity();//variance is very small so Ap's position will not be changed 
}
...
void WifiEkf::update(double z)
{
    ...
    ROS_INFO_STREAM("wifi measurement: " << z << measurement(p, q, ap));
...
//update with more than one AP
void WifiEkf::update(Eigen::VectorXd zz)
{
    ROS_INFO_STREAM("P1: " << P.diagonal().transpose());
    Eigen::VectorXd y(zz.size());
    Eigen::MatrixXd Hp(zz.size(), 12);
    for(int i = 0; i < zz.size(); i++)
    {
    	y(i) = zz(i) - measurement(p, q, APs[i]);
    	ROS_INFO_STREAM("wifi measurement: " << zz(i) << "  " << measurement(p, q, APs[i]));
   		if(std::isnan(zz(i)))//zz(i) = nan
   		{
   			ROS_INFO_STREAM("nan measurement");
   			return;//skip this update 
   		}
   		Eigen::MatrixXd Hp_  = jacobian(i);//myJacobian(delta_p, delta_theta, delta_ap);
   		//Eigen::MatrixXd Hp_ = myJacobian(delta_p, delta_theta, delta_ap, i);
   		Hp.row(i) = Hp_;
    } 
    
    
    Eigen::MatrixXd s = Hp * P * Hp.transpose() + W_cov*Eigen::MatrixXd::Identity(zz.rows(), zz.rows());
    
    Eigen::MatrixXd K = P * Hp.transpose() * s.inverse();
    
    P = (Eigen::MatrixXd::Identity(12, 12) - K * Hp) * P;
...
Eigen::Matrix<double, 1, 12> WifiEkf::myJacobian(const Eigen::Vector3d &_delta_p, const Eigen::Vector3d &_delta_theta, const Eigen::Vector3d &_delta_ap, int ap_id)
{
    Eigen::VectorXd ap = APs[ap_id];
    Eigen::Matrix<double, 1, 12> J = Eigen::Matrix<double, 1, 12>::Zero();
	//double C = -2*M_PI*R/LANDA;
	double C = -1;
	double N = (p(0) - ap(0) ) * (q(0,0) + _delta_theta(2)*q(0,1) - _delta_theta(1)*q(0,2) ) + (p(1) - ap(1)) * (q(1,0) + _delta_theta(2)*q(1,1) - _delta_theta(1)*q(1,2) ) + (p(2) - ap(2)) * (q(2,0) + _delta_theta(2)*q(2,1) - _delta_theta(1)*q(2,2) );
...
//id of ap
Eigen::Matrix<double, 1, 12> WifiEkf::jacobian(int ap_id)
{
    Eigen::VectorXd ap = APs[ap_id];
    Eigen::Vector3d eps_v[3];
    eps_v[0] = Eigen::Vector3d(EPS, 0.0, 0.0);
